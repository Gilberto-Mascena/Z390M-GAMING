## _**Changes in this Version**_

- ### 🎉 _Updates in this release:_    
    - #### _Opencore_
        - _Version 1.0.7_  

    - #### Drivers
        - _AudioDxe_
        - _OpenCanopy_
        - _OpenRuntime_
        - _ResetNvramEntry_

    - #### _Kexts_
        - _AppleALC 1.9.7_    
        - _Lilu 1.7.2_
        - _NVMeFix 1.1.3_
                
    - #### Tools
        - _OpenShell_



> [!IMPORTANT]
> _Z390M-GAMING-WIFI-BROADCOM-VENTURA-BEFORE_
> - _For Catalina to Ventura with native Broadcom:_
>
> _Z390M-GAMING-WIFI-BROADCOM-SONOMA:_
> - _For Sonoma, then run [OCLP](https://github.com/dortania/OpenCore-Legacy-Patcher/releases). Always use the latest version_  

> - _macOS Tahoe:_ 
> _After system installation, enable WhateverGreen in config.plist_.
>
> - _Audio codec:_
> _For support in Tahoe, see [Emilio P Egido](https://github.com/perez987/AppleHDA-back-on-macOS-26-Tahoe)_

- ### _[WIFI supported](https://dortania.github.io/Wireless-Buyers-Guide/unsupported.html#supported-chipsets)_
